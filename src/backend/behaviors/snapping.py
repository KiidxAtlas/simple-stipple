...
def snap_circle_rim(
    ...
    *,
    ...
    """Snap a circle's radius so the rim passes through a nearby vertex or midpoint.

    Works by checking whether any existing vertex/midpoint lies within a pixel-width
    band around the current radius.  If so, the cursor is projected to that exact
    radius in the same angular direction, so the drawn circle will pass through it.
    """
    current_r = math.hypot(wx - center_wx, wy - center_wy)
    if current_r < _EPS:
        return None
    dir_x = (wx - center_wx) / current_r
    dir_y = (wy - center_wy) / current_r

    best_dist_px = snap_dist
    best_r: float | None = None
    best_kind = "circle_rim"

    for pi, poly in enumerate(polylines):
        if pi in hidden_polys or len(poly) < 1:
            continue
        # Check vertices
        for pt in poly:
            r_to_pt = math.hypot(pt[0] - center_wx, pt[1] - center_wy)
            if r_to_pt < _EPS:
                continue
            diff_px = abs(r_to_pt - current_r) * scale
            if diff_px < best_dist_px:
                best_dist_px = diff_px
                best_r = r_to_pt
                best_kind = "circle_rim"
        # Check midpoints
        n = len(poly)
        for vi in range(n - 1):
            ...
            bx, by = poly[vi + 1]
            mx, my = (ax + bx) / 2.0, (ay + by) / 2.0
            r_to_mid = math.hypot(mx - center_wx, my - center_wy)
            if r_to_mid < _EPS:
                continue
            diff_px = abs(r_to_mid - current_r) * scale
            if diff_px < best_dist_px:
                best_dist_px = diff_px
                best_r = r_to_mid
                best_kind = "circle_rim"
    ...
    if best_r is not None:
        snapped_wx = center_wx + dir_x * best_r
        snapped_wy = center_wy + dir_y * best_r
        return (snapped_wx, snapped_wy, best_kind)
    return None
