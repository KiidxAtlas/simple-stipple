...
from src.backend.dxf.fvi import FviExportOptions, write_fvi
...
def export_laserstar_package(
    ...
    *,
    ...
    report = write_fvi(
        _records(vector_polys), vector_path,
    ...
    if raster_source and raster_spec:
        ...
        x, y, w, h = (
            raster_spec.x_mm, raster_spec.y_mm, raster_spec.width_mm, raster_spec.height_mm
        )
        frame = [[(x, y), (x + w, y), (x + w, y + h), (x, y + h), (x, y)]]
        write_fvi(
            _records(frame), folder / "03_placement-reference.fvi",
    ...
    setup = [
        f"JOB: {safe_name}", "", "LASERSTAR TARGET",
        f"Machine: {profile.machine}", f"Software: {profile.software}",
        f"Laser: {profile.laser}", f"Lens: {profile.lens_mm} mm",
        "", "VECTOR IMPORT", "1. Import 01_pattern-and-outline.fvi into StarFX.",
    ...
    manifest = {
        ...
        "vector_report": {
            "paths": report.path_count, "travel_mm": report.travel_mm,
            "bounds_mm": report.bounds_mm, "warnings": list(report.warnings),
        },
