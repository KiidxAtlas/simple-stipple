...
class TraceCancelled(Exception):
    """Raised by image_to_outlines() when cancel_check() reports a newer
    retrace has superseded this one — lets a stale background trace stop
...
def _build_mask(
    ...
    if supersample > 1:
        h, w = gray.shape[:2]
        gray = cv2.resize(
            gray, (w * supersample, h * supersample), interpolation=cv2.INTER_CUBIC
        )
...
def image_to_outlines(
    ...
    *,
    ...
    if edge_mode:
        ...
        mask = _build_mask(
            bgr, blur_radius, threshold, invert, close_radius, _MASK_SUPERSAMPLE
        )
    _check_cancelled()
    ...
    polys: list[Poly] = [
        [
            (float(p[0][0]) / _MASK_SUPERSAMPLE, float(p[0][1]) / _MASK_SUPERSAMPLE)
            for p in c
        ]
