...
def main(argv: list[str] | None = None, *, run_app: Callable[[argparse.Namespace], int]) -> int:
    """Generic bootstrap: parse args, prep the cache dir, configure logging,
    then hand off to ``run_app`` (injected — see module docstring for why
    this doesn't just import ``simple_stipple.app.launcher.run_app`` directly)."""
    ...
    from simple_stipple.platform.error_reporting import install_excepthook
