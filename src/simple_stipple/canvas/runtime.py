...
class CanvasRuntime:
    ...
    def rename_shape(self, layer_name: str, shape_key: object, new_label: str) -> None:
        ...
        if isinstance(shape_key, (tuple, list)) and shape_key:
            first = shape_key[0]
            gid = self._canvas._grouping_service.group_of(first) if isinstance(first, str) else None
            if gid is not None:
...
# Pattern and Trace page runtimes live in their respective feature packages.
