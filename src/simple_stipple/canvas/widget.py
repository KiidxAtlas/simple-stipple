...
from simple_stipple.platform.config import DEFAULT_RADIAL_MENU_TOOLS
...
class DxfCanvas(CanvasView):
    ...
    def _rightclick_cb(self, cx: float, cy: float) -> None:
        ...
        self._add_context_section(
            ...
            "create",
            section_enabled,
            lambda: self._build_create_shape_menu(menu, section_enabled, poly_hit_early),
        )

        poly_hit = poly_hit_early
        if poly_hit is not None:
            self._build_entity_actions(menu, poly_hit, section_enabled, _run_transform)
            menu.addSeparator()
        ...
        self._add_context_section(
            ...
            "selected",
            section_enabled,
            lambda: self._build_selection_actions(
                menu,
                section_enabled,
                _run_transform,
        ...
        self._add_context_section(
            menu, "selection", section_enabled, lambda: self._build_selection_tools(menu)
        )
        self._add_context_section(
            menu,
            "share_diagnostics",
            section_enabled,
            lambda: self._build_share_actions(menu, _run_transform),
        )
        self._add_context_section(
            menu,
            "boolean",
            section_enabled,
            lambda: self._build_boolean_actions(menu, section_enabled),
        )
        self._add_context_section(
            menu,
            "arrange",
        ...
        self._add_context_section(
            menu, "view", section_enabled, lambda: self._build_view_actions(menu)
        )

        self._apply_context_menu_overflow(menu)
        menu.popup(self.mapToGlobal(QPoint(int(cx), int(cy))))

    def _apply_context_menu_overflow(self, menu: QMenu) -> None:
        """Reorder and potentially overflow context menu sections."""
