...
from simple_stipple.engine.geometry.spatial import build_snap_tree, find_nearest_index
...
def _polyline_segments(
    ...
    for entity_id, polyline in polylines:
        ...
        for index in range(count):
            if (entity_id, index) in excluded:
                continue
            segments.append(
                (entity_id, index, polyline[index], polyline[(index + 1) % len(polyline)])
            )
    return segments
...
def _nearest_midpoint(
    ...
    best_distance = snap_dist
    best_point: Point | None = None
    for _entity_id, _index, first, second in segments:
        midpoint = ((first[0] + second[0]) / 2.0, (first[1] + second[1]) / 2.0)
        screen_x, screen_y = w2c(*midpoint)
        distance = math.hypot(cx - screen_x, cy - screen_y)
        if distance < best_distance:
            best_distance, best_point = distance, midpoint
    return None if best_point is None else (best_distance, best_point)
...
def _nearest_intersection(
    ...
    best_distance = snap_dist
    best_point: Point | None = None
    for index, (_eid, _si, first_a, second_a) in enumerate(segments):
        for _other_eid, _other_si, first_b, second_b in segments[index + 1 :]:
            intersection = segment_intersection_point(first_a, second_a, first_b, second_b)
            if intersection is None:
                continue
            screen_x, screen_y = w2c(*intersection)
            distance = math.hypot(cx - screen_x, cy - screen_y)
            if distance < best_distance:
                best_distance, best_point = distance, intersection
    return None if best_point is None else (best_distance, best_point)
...
def snap_to_polyline(
    ...
    *,
    ...
    segments = _polyline_segments(candidate_polys, is_poly_closed)
    ...
    if allow_edge:
        ...
        if perp_ref is not None:
            ...
            for _eid, _index, first, second in segments:
                eax, eay = first
                ebx, eby = second
