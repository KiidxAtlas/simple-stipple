...
def _rotation_matrix(args: list[float]) -> Matrix:
    ...
    if len(args) < 3:
        return rotation
    center_x, center_y = args[1], args[2]
    translate_to_center: Matrix = (1, 0, 0, 1, center_x, center_y)
    translate_from_center: Matrix = (1, 0, 0, 1, -center_x, -center_y)
    return _matrix_multiply(
        _matrix_multiply(translate_to_center, rotation),
        translate_from_center,
    )


def _transform_matrix(name: str, args: list[float], payload: str) -> Matrix:
    if name == "matrix" and len(args) == 6:
        return cast(Matrix, tuple(args))
    if name == "translate" and args:
        return (1, 0, 0, 1, args[0], args[1] if len(args) > 1 else 0)
    if name == "scale" and args:
        return (args[0], 0, 0, args[1] if len(args) > 1 else args[0], 0, 0)
    if name == "rotate" and args:
        return _rotation_matrix(args)
    if name == "skewx" and args:
        return (1, 0, math.tan(math.radians(args[0])), 1, 0, 0)
    if name == "skewy" and args:
        return (1, math.tan(math.radians(args[0])), 0, 1, 0, 0)
...
def _parse_transform(value: str) -> Matrix:
    ...
    for name, payload in operations:
        ...
        name = name.lower()
        result = _matrix_multiply(result, _transform_matrix(name, args, payload))
    return result
...
def svg_to_dxf(
    ...
    *,
    ...
    :func:`simple_stipple.engine.formats.dxf.write_polylines_dxf` — the single DXF writer,
