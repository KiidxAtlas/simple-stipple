...
def _with_segment_end(
    points: list[Point], ref: tuple[Point, Point, int, int], value: Point
) -> list[Point]:
...
def solve_constraints(
    ...
    for _ in range(max(1, passes)):
        for constraint in constraints:
            ...
            if (
                constraint.kind == "coincident"
                and "point_vertex" in constraint.parameters
                and len(ids) >= 2
            ):
            ...
            if constraint.kind == "symmetric" and len(ids) >= 3:
                ...
                if (
                    ...
                    continue
                updated = list(second_points)
                updated[second_vertex] = _reflect_across_line(
                    first_points[first_vertex], axis_ref[:2]
                )
                solved[ids[2]] = updated
                continue
            if constraint.kind == "smooth" and constraint.parameters.get("curve") and len(ids) >= 2:
                ...
                if (
                    ...
                    or second_points is None
                    or len(first_points) < 4
                    or len(second_points) < 3
                ):
                    continue
        ...
        if (
            ...
            try:
                ...
                if first_ref is None or point_points is None:
                    raise IndexError
                residuals[constraint.id] = math.dist(
                    point_points[point_vertex],
                    _project_onto_line(point_points[point_vertex], first_ref[:2]),
                )
            except (TypeError, ValueError, IndexError):
                residuals[constraint.id] = math.inf
            continue
