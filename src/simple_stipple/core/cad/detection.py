...
def detect_primitive(points: list[Point], *, tolerance: float = 0.015) -> DetectedShape | None:
    ...
    if len(vertices) >= 16:
        return DetectedShape("circle", {"center": center, "radius": radius}, confidence)
...
def convert_to_parametric(entity: Any, detected: DetectedShape | None = None) -> Any:
    """Return an editable parametric copy of a detected polyline entity."""
    shape = detected or detect_primitive(list(entity.points))
    if shape is None:
        raise ValueError("Entity is not a detected parametric shape")
