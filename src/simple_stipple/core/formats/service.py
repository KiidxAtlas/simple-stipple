"""DXF/FVI/SVG import-export service for the UI layer.

``DxfService`` exposes plain-data import, export, repair, and conversion
operations over the format modules in this package, so feature and UI code
reaches one facade instead of assembling format internals itself.
"""
...
class DxfService:
    ...
    def fix_dxf(
        ...
        *,
        ...
        return fix_dxf(input_path, output_path, mode=mode)
    ...
    def write_polylines_svg(
        ...
        """Write polylines to an SVG file. Returns ``{"polylines", "width_mm", "height_mm"}``."""
        return write_polylines_svg([list(p) for p in polylines], filepath)
    ...
    def write_fvi(
        ...
        """Write data to an FVI file, applying the given export options."""
        return write_fvi(data, filepath, options)

    @staticmethod
    def read_fvi(filepath: str) -> FviDocument:
        """Read an FVI file."""
        return read_fvi(filepath)
    ...
    def render_fvi(
        ...
        return render_fvi(data, options)
