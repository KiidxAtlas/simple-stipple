...
def gen_hilbert_curve(
    ...
    order controls complexity; points count is (2^order)^2.
    """
    ...
    # Keep curve inside a margin within the outline bbox.
...
def gen_reaction_diffuse(
    ...
    # Seed blobs (center + random spots)
    cy, cx = ny // 2, nx // 2
    ...
    # Gray-Scott params tuned for labyrinth-like textures.
    dA = 1.0
    dB = 0.5
    feed = 0.0367
    kill = 0.0649
    ...
    # Extract iso-segments (marching-squares-ish) from B field.
    t = float(threshold)
    ...
    for iy in range(ny - 1):
        ...
        for ix in range(nx - 1):
            ...
            if len(pts) == 2:
                _collect_lines(
                    outline_poly.intersection(LineString([pts[0], pts[1]])), result
                )
            elif len(pts) == 4:
                _collect_lines(
                    outline_poly.intersection(LineString([pts[0], pts[1]])), result
                ...
                _collect_lines(
                    outline_poly.intersection(LineString([pts[2], pts[3]])), result
                )
