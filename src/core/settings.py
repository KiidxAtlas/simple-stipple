...
# Command in src/ui/canvas/interaction/commands.py — its default lives on Command.shortcut,
# not here, so there is exactly one place that owns each shortcut's default.
# src.ui.canvas.interaction.commands.apply_keybindings() reads the "keybindings" dict
...
# order. Every wedge id is a real src.ui.canvas.interaction.commands.Command id, so the
...
# sections show and in what order. Consumed by src.ui.widgets.canvas.draw_sidebar
