...
class Command:
    ...
    hidden: bool = False
    # Only deterministic editing operations should become Repeat Last.
    # Modes, view toggles, selection changes, and clipboard-only actions leave
    # the previous repeatable operation intact.
...
def _create_procedural(name: str):
    return lambda view: view.create_procedural_primitive(name)
...
COMMANDS: tuple[Command, ...] = (
    ...
    Command(
        "edit.array_path",
        "Array — Along Path…",
        lambda v: v._array_duplicate_along_path(),
        category="Edit",
    ...
    Command(
        ...
        lambda v: v.create_symbol_from_selection(),
        category="Symbols",
        when=lambda v: bool(v._sel),
    ),
    Command(
    ...
    Command(
        "draft.coordinate_entry",
        "Enter Coordinate…",
        lambda v: v.show_coordinate_entry(),
        category="Drafting",
        requires_selectable=False,
    ),
    Command(
        "construct.xline",
        "Construction Line from Segment",
        lambda v: v.construction_line_from_selection(),
        category="Construction",
        when=lambda v: len(v._sel) == 1,
    ),
    Command(
        "construct.ray",
        "Construction Ray from Segment",
        lambda v: v.construction_line_from_selection(ray=True),
        category="Construction",
        when=lambda v: len(v._sel) == 1,
    ),
    Command(
        "construct.bisector",
        "Angle Bisector",
        lambda v: v.create_angle_bisector(),
        category="Construction",
        when=lambda v: len(v._sel) == 2,
    ),
    Command(
        "construct.centerline",
        "Centerline Between Edges",
        lambda v: v.create_centerline(),
        category="Construction",
        when=lambda v: len(v._sel) == 2,
    ),
    Command(
        ...
        lambda v: v.create_circle_through_three_points(),
        category="Construction",
        when=lambda v: bool(v._sel),
    ),
    Command(
        ...
        lambda v: v.create_tangents_from_point(),
        category="Construction",
        when=lambda v: len(v._sel) == 2,
    ),
    Command(
        ...
        lambda v: v.create_common_circle_tangents(),
        category="Construction",
        when=lambda v: len(v._sel) == 2,
    ),
    Command(
    ...
    Command(
        ...
        "Constrain Endpoints Coincident",
    ...
    Command(
        ...
        lambda v: v.prompt_morph_selected_paths(),
        category="Path",
        when=lambda v: len(v._sel) == 2,
    ),
    Command(
        "path.recognize_shapes",
        "Recognize Parametric Shapes",
        lambda v: v.recognize_selected_shapes(),
        category="Path",
        when=lambda v: bool(v._sel),
    ),
    Command(
        ...
        lambda v: v.reverse_selected_paths(),
        category="Path",
        repeatable=True,
        when=lambda v: bool(v._sel),
    ),
    Command(
        "path.set_start",
        "Set Path Start Near Cursor",
        lambda v: v.set_selected_path_start(),
        category="Path",
        when=lambda v: len(v._sel) == 1,
    ),
    Command(
        ...
        "Resample by Spacing…",
        lambda v: v.prompt_resample_spacing(),
        category="Path",
        when=lambda v: bool(v._sel),
    ),
    Command(
        ...
        lambda v: v.prompt_resample_count(),
        category="Path",
        when=lambda v: bool(v._sel),
    ),
    Command(
        "path.fit_line",
        "Fit to Line",
        lambda v: v.fit_selected_to_primitive("line"),
        category="Path",
        when=lambda v: bool(v._sel),
    ),
    Command(
        "path.fit_circle",
        "Fit to Circle",
        lambda v: v.fit_selected_to_primitive("circle"),
        category="Path",
        when=lambda v: bool(v._sel),
    ),
    Command(
        "path.fit_arc",
        "Fit to Arc",
        lambda v: v.fit_selected_to_primitive("arc"),
        category="Path",
        when=lambda v: bool(v._sel),
    ),
    Command(
        "shape.ring",
        "Create Ring",
        _create_procedural("ring"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
        "shape.gear",
        "Create Gear / Sprocket",
        _create_procedural("gear"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
    ...
    Command(
        "shape.keyhole",
        "Create Keyhole",
        _create_procedural("keyhole"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
        "shape.superellipse",
        "Create Superellipse / Squircle",
        _create_procedural("superellipse"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
        "shape.rounded_star",
        "Create Rounded Star",
        _create_procedural("rounded_star"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
        "shape.chamfered_star",
        "Create Chamfered Star",
        _create_procedural("chamfered_star"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
        "shape.finger_joint_box",
        "Create Finger-Joint Box",
        _create_procedural("finger_joint_box"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
        "shape.dovetail_box",
        "Create Dovetail Box",
        _create_procedural("dovetail_box"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
        "shape.tabbed_panel",
        "Create Tabbed Panel",
        _create_procedural("tabbed_panel"),
        category="Shapes",
        requires_selectable=False,
    ),
    Command(
        "shape.polygon_from_edge",
        "Regular Polygon from Selected Edge…",
        lambda v: v.prompt_polygon_from_edge(),
        category="Shapes",
        when=lambda v: len(v._sel) == 1,
    ),
    Command(
    ...
    Command(
        "mode.knife",
        "Knife Tool",
        lambda v: v.set_mode("knife" if v.get_mode() != "knife" else "select"),
        category="Modes",
    ),
    Command(
        "measure.toggle",
    ...
    Command(
        ...
        _select_draw_primitive("rounded_rectangle"),
        category="Modes",
    ),
    Command(
        "canvas.circle",
    ...
    Command(
        "view.geometry_health",
        "Geometry Health Overlay",
        lambda v: v.set_geometry_health_visible(not v._geometry_health_visible),
        "Ctrl+Shift+H",
        category="View",
        requires_selectable=False,
    ),
    Command(
        ...
        lambda v: v.set_curvature_visible(not v._curvature_visible),
        "Ctrl+Shift+K",
        category="View",
        requires_selectable=False,
    ),
...
def can_run(view: Any, cmd_id_or_cmd: str | Command) -> bool:
    """Return whether a command is available in the current canvas state.

    Menu builders naturally work with registry IDs while keyboard dispatch
    already holds resolved ``Command`` objects, so accept both at this public
    boundary just like :func:`run` does.
    """
...
def run(view: Any, cmd_id_or_cmd: str | Command) -> bool:
    ...
    cmd.run(view)
    if cmd.repeatable:
        view._last_repeat_action = None
        view._last_command_id = cmd.id
    return True


def repeat_last(view: Any) -> bool:
    """Repeat the last registry command when it remains valid in context."""
    repeat_action = getattr(view, "_last_repeat_action", None)
    if isinstance(repeat_action, tuple) and len(repeat_action) == 2 and callable(repeat_action[1]):
        repeat_action[1]()
        return True
    command_id = getattr(view, "_last_command_id", None)
    if not isinstance(command_id, str) or command_id not in _BY_ID:
        view._show_flash("No command to repeat", 900)
        return False
    command = _BY_ID[command_id]
    if not can_run(view, command):
        view._show_flash(f"Cannot repeat {command.label} here", 1100)
        return False
    command.run(view)
    return True
