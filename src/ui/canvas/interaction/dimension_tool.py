...
from src.ui.canvas.interaction.tools import DimensionTool as DimensionBackend
...
class DimensionTool(DimensionBackend):
    ...
    def _vertex_at(self, cx: float, cy: float) -> dict[str, Any] | None:
        ...
        if hit is None:
            return None
        entity_index, vertex_index = hit
        entity = self.v._entities[entity_index]
        return {
            "kind": "vertex",
            "entity_id": entity.id,
    ...
    def _vertex_from_ref(self, reference: dict) -> tuple[int, int, tuple[float, float]] | None:
        entity_id = str(reference.get("entity_id", ""))
        vertex_index = int(reference.get("vertex_index", -1))
        entity_index = next(
            (index for index, entity in enumerate(self.v._entities) if entity.id == entity_id),
            None,
        )
        if entity_index is None or not (
            0 <= vertex_index < len(self.v._entities[entity_index].points)
        ):
            return None
        return entity_index, vertex_index, self.v._entities[entity_index].points[vertex_index]
    ...
    def set_value(self, index: int, target: float) -> bool:
        ...
        if isinstance(driving, dict) and driving.get("kind") == "circle_diameter":
            ...
            if entity_index is None or target <= 0:
                return False
            entity = deepcopy(self.v._entities[entity_index])
        ...
        entity = deepcopy(self.v._entities[second[0]])
