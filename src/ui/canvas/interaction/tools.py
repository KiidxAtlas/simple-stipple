...
from src.backend.cad.geometry import arc_from_center_start_end, arc_from_three_points
from src.backend.editing.transform import rotate, translate
from src.core.settings import DEFAULT_RADIAL_MENU_TOOLS, RADIAL_MENU_SHORT_LABELS
...
def start_bezier_handle_drag(v: CanvasView, hit: tuple[int, int, str]) -> bool:
    entity_index, _anchor_index, _side = hit
    if v._is_locked(entity_index):
        return False
    v._bezier_handle_drag = hit
...
def apply_bezier_handle_drag(v: CanvasView, event: QMouseEvent) -> bool:
    ...
    if hit is None:
        return False
    entity_index, anchor_index, side = hit
    pos = event.position()
    wx, wy = v._c2w(pos.x(), pos.y())
    anchor = v._entities[entity_index].points[anchor_index]
    if event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
        wx, wy = v._angle_snap(*anchor, wx, wy)
    current = next(
        tip
        for vi, handle_side, tip in v._bezier_handles(entity_index)
...
class DimensionTool(CanvasTool):
    ...
    def _segment_at(self, cx: float, cy: float) -> dict | None:
        ...
        best: dict | None = None
        best_distance = 9.0
        for entity_index in range(len(v._entities)):
            if not v._entity_selectable(entity_index):
                continue
            points = v._flattened_points(entity_index)
            ...
            best = {
                "key": (v._entities[entity_index].id, segment_index),
    ...
    def _segment_from_ref(self, reference: dict) -> dict | None:
        ...
        entity_index = next(
            (index for index, entity in enumerate(v._entities) if entity.id == entity_id), None
        )
        if entity_index is None:
            return None
        points = v._entities[entity_index].points
    ...
    def set_value(self, index: int, target: float) -> bool:
        ...
        if kind == "segment_length" and len(segments) == 1:
            segment = segments[0]
            entity = deepcopy(v._entities[segment["entity_index"]])
            ...
            entity = deepcopy(v._entities[segments[1]["entity_index"]])
            if segments[0]["entity_index"] == segments[1]["entity_index"]:
    ...
    def press(self, event: QMouseEvent) -> bool:
        ...
        # annotation without requiring endpoint placement.
        if v._dim_pending_p1 is None:
            entity_index = next(
                (
                    index
                    for index, candidate in enumerate(v._entities)
            ...
            if entity_index is not None:
                entity = v._entities[entity_index]
...
class EditTool(CanvasTool):
    ...
    def double_click(self, event: QMouseEvent) -> bool:
        ...
        if hit is not None:
            ...
            if v._is_locked(pi):
                return True
            entity = deepcopy(v._entities[pi])
...
class SelectTool(CanvasTool):
    ...
    def press(self, event: QMouseEvent) -> bool:
        ...
        if (
            ...
            previous = next((index for index in candidates if index in v._sel), None)
            target = (
                candidates[0]
                if previous is None
        ...
        if target is not None:
            ...
            if gid is not None:
                members = {i for i, e in enumerate(v._entities) if e.group == gid and not e.hidden}
            ...
            if (
                ...
                and target_kind not in {"arc", "circle", "ellipse", "rectangle", "polygon", "slot"}
                    ...
                    if abs(step_dx) > 1e-12 or abs(step_dy) > 1e-12:
                        for idx in v._sel:
                            ...
                            v._entities[idx].points = [
                                (x + step_dx, y + step_dy) for x, y in v._entities[idx].points
                            ]
        ...
        if v._lasso_active:
            ...
            if len(points) >= 3:
                try:
                    ...
                    for idx, entity in enumerate(v._entities):
                        if not v._entity_selectable(idx) or not entity.points:
                            continue
            ...
            if gids:
                picked |= {
                    i
                    for i, entity in enumerate(v._entities)
                    if entity.group in gids and v._entity_selectable(i)
                }
    ...
    def double_click(self, event: QMouseEvent) -> bool:
        ...
        if hit is not None:
            ...
            if profile:
                v._sel = profile
                v._show_flash(f"Selected enclosed profile · {len(profile)} edge(s)", 1200)
                v._redraw()
                v._notify()
            else:
