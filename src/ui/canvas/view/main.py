...
from src.ui.canvas.tools.select import SelectionService
from src.ui.canvas.view.commands import (
    _cancel_active_drag,
    _escape_cb,
    _find_dimension_at,
    _rightclick_cb,
...
from src.ui.canvas.view.config import _initialize_view
from src.ui.canvas.view.helpers import (
    _animate_view_to,
    _background_edit_hit,
    _chamfer_vertex,
    _connected_poly_ids,
    _connected_poly_indices,
...
from src.ui.canvas.view.interactions import (
    _append_dimension,
    _clear_dimensions,
...
class CanvasView(
    ...
    def _delete_poly(self, entity_id: str) -> None:
        if (entity := self._entities_by_id.get(entity_id)) is None:
            return
        drop = {entity_id}
    ...
    def set_hidden_ids(self, entity_ids: list[str]) -> None:
        new_hidden = {
            eid for eid in entity_ids if (entity := self._entities_by_id.get(eid)) is not None
        }
    ...
    def _bbox(self) -> tuple[float, float, float, float]:
        pts = [
            ...
            if (entity := self._entities_by_id[eid]).kind not in {"xline", "ray"}
...
CanvasView._connected_poly_ids = _connected_poly_ids
CanvasView._connected_poly_indices = _connected_poly_indices
