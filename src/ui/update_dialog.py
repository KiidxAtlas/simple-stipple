...
class UpdateDialog(QDialog):
    ...
    def _build_update_available_ui(self, layout: QVBoxLayout, info: UpdateInfo) -> None:
        ...
        download_btn.setMinimumWidth(130)
        download_btn.setProperty("role", "primary")
        download_btn.clicked.connect(lambda: self._download_and_install(info))
    ...
    def _open_release_page(self, info: UpdateInfo) -> None:
        ...
        url = f"https://github.com/{info.url.split('github.com/')[1].split('/releases/')[0]}/releases"
        webbrowser.open(url)
    ...
    def _download_and_install(self, info: UpdateInfo) -> None:
        ...
        progress.setText(f"Downloading update to:\n{download_path}")
        progress.setStandardButtons(QMessageBox.StandardButton.NoButton)
        progress.show()

        # Download
        success = download_update(info.url, download_path)
        progress.close()
