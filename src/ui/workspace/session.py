...
def collect_workspace_document(
    *,
    ...
    return build_workspace_document(
        ...
        tab_states={
            page_id: page.get_workspace_state() for page_id, page in workspace_pages
        },
        preset_state={
            page_id: page.get_preset_state() for page_id, page in preset_pages
        },
...
def recent_workspace_paths(settings: dict) -> list[Path]:
    return [
        Path(path)
        for path in settings.get("recent_workspaces", [])
        if Path(path).exists()
    ]
