...
- `canvas` — the reusable interactive vector editor: runtime, rendering, tools, operations,
  view behavior, canvas widgets, dialogs for configuring canvas behavior, and layer tree.
...
2. Engine imports no app, canvas, feature, or UI subsystem, and no document
   submodule except `document.identity` — a dependency-free primitive-type
   module (`EntityId = str`, `new_entity_id()`) explicitly documented as
   shared across all layers. Engine must never import `document.model`,
   `document.service`, or any other document submodule.
...
`ui` must never import `canvas` — dialogs that configure canvas behavior (keybindings,
canvas customization, canvas text tool) live in `canvas/dialogs/`, not `ui/dialogs/`, so
`ui` stays a leaf that `canvas` can depend on without a cycle back.
...
| a dialog whose only purpose is configuring canvas behavior             | `canvas/dialogs/`                                   |
...
two independent consumers exist or when it represents an explicit shared aggregate. Do not
split a file solely because it is long — only split to fix a genuine dependency-boundary
violation or duplication; this project favors fewer files.
