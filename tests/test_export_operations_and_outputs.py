...
def dispose_test_windows(app: QApplication):
    """Do not leak closed PatternPage top levels into the next Qt test module.

    ``close()`` only hides a parentless widget.  These tests create
    ``PatternPage`` instances without a parent, so leaving them hidden makes
    later fixtures treat them as application-owned windows and eventually
    trips PySide's native item teardown.
    """
